# 知识库设计（Knowledge Base Design）V1
...
> **文档范围说明**：本文档**仅覆盖知识域（Knowledge Domain）**——历史的、只读的、有出处的内容。
> 运行时域（Project / Session / Amendment / TextVersion 等课堂产出对象）见《总体设计V2》第七章，本文档不重复。
...
| 8 | `TeachingIssue` | **教学议题** | **策展** | **L2** | **5–8 / 场景** |
| 9 | `IssueOption` | **方案区间** | **策展** | **L2** | **15–24 / 场景** |
| 10 | `Proposal` | 历史提案 | 抽取 | L1 | 300–800 |
| 11 | `Country` | 国家/关税区 | 人工配置 | — | 50+ |
| 12 | `CountryPosition` | **国家立场卡** | **策展** | **L2** | **约 90 / 场景** |
| 13 | `PositionEvolution` | **立场演变节点** | **策展** | **L2** | **约 300 / 场景** |
| 14 | `CountryProfile` | **国家画像** | **策展** | **L2** | **12–18 / 场景** |
| 15 | `ProfileMetric` | 画像指标 | 数据导入 | L1 | 约 300 |
| 16 | `Outcome` | 谈判结果 | 策展 | L2 | 5–8 / 场景 |
| 17 | `ReviewLog` | 审核留痕 | 自动 | — | 动态 |

> **加粗行是教学效果的真正来源**，合计约 400 条人工校准记录，是项目的关键路径。
...
> 通用字段（所有对象均含，下表不再重复）：
> `id`（主键）、`created_at`、`updated_at`、`created_by`、`kb_version`（所属知识库发布版本）
...
> **三语去重规则**：以 `doc_symbol` 为唯一键，同编号的多语言版本合并为一组，`is_primary_language=true` 的进入检索库，其余仅保留文件路径供查阅。这可能使实际入库规模显著低于原始下载份数（见附录 A 待确认事项）。
...
| **三语重复污染检索** | 同一文件三个语言版本重复召回 | `is_primary_language` 过滤 |
| **段落定位错误** | OCR 或无编号文件定位偏差 | `para_confidence` 标注；低置信度标「近似位置」 |
| **策展工时超预算** | 约 400 条 L2 记录是关键路径 | 矩阵进度追踪；优先完成参与国 × 议题的核心格子，非参与国延后 |
...
| 对象 | 数量级 | 生产方式 | 工时敏感度 |
|---|---|---|---|
| `Document` | 待定（三语去重后） | 自动 | 低 |
| `Chunk` | 10 万级 | 自动 | 低 |
| `Issue`（候选） | 1000+ | 自动 | 低 |
| `TeachingIssue` | WTO 5–6 + IOTC 3–5 | **策展** | 中 |
| `IssueOption` | 约 30 | **策展** | 中 |
| `CountryPosition` | 议题数 × 参与国数 ≈ 90 | **策展** | **高** |
| `PositionEvolution` | 每卡 3–4 节点 ≈ 300 | **策展** | **高** |
| `CountryProfile` | 12–18 | **策展** | 中 |
| `ProfileMetric` | 每国约 20 项 ≈ 300 | 数据导入 + 校对 | 中 |
| `Outcome` | 8–10 | **策展** | 低 |

> **关键路径为 `CountryPosition` + `PositionEvolution`，合计约 400 条。** 排期应以此为基准，而非以 PDF 抽取量为基准。
...
| # | 事项 | 影响 | 状态 |
|---|---|---|---|
| 1 | WTO 文件三语去重后的实际 `doc_symbol` 数量 | `Document`/`Chunk` 规模、抽取排期 | 待数据同学核实 |
| 2 | 教学议题最终数量（V6 表格 6 项 / 正文 5 项） | `TeachingIssue` 与立场卡矩阵大小 | 待教师确认 |
| 3 | 参与国最终清单（WTO 场景 / IOTC 场景各一份） | **立场卡数量 = 议题数 × 国家数，关键路径** | 待教师确认 |
| 4 | IOTC 第一节选用的具体议题 | IOTC 语料筛选与议题卡产出 | 待教师确认 |
