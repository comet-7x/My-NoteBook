...
2. **结构统一**：建议格式为 `[分支类型]/[核心描述]-[可选ID]`（斜杠分隔类型和内容，连字符分隔单词）；
3. **语义清晰**：描述部分简洁（2-5 个单词），直接体现用途（如功能、bug、版本）；
4. **可追溯性**：优先包含任务 ID（Jira、GitHub Issues 编号），便于关联需求 / 缺陷；
5. **长度适中**：避免过长（如超过 50 字符），核心信息前置。
