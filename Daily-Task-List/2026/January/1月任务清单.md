...
## 1月30日：
- [ ] 1. 需要根据近几年的数据进行总结和预测和趋势推理（需要有例如：增长百分点数据），需要基于现有数据进行总结提炼
- [ ] 2. 检索词汇，例如小龙虾（龙虾，等品种），需要一个问题重写？比如AI必须知道例如（小龙虾的学名叫克氏原螯虾），需要有检索词的联想
- [ ] 3. 提示词的建设，需要让大模型预测用户想要问什么样得到问题。
- [ ] 4. 前期的时候，预设回复的答案，后期根据大部分的答案进行回答。（根据用户的反馈，确定关联词汇有哪些。）
