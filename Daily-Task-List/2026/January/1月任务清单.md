...
## 1月23日：
- [x] 1. 解决最后一个字符为双引号的问题。
- [x] 2. 前端增加代码块的复制（以三个`包裹），同时增加渲染一个`包括的代码内容
- [ ] 3. 在选择知识库和分区以后进行聊天框内部依然支持选择
- [ ] 4. 在选择分区以后支持点×删除
- [ ] 5. 用户快速发送无效消息，应该等待模型回答完成或者点击停止后才能继续问答
- [ ] 6. 进入全部历史记录窗口以后，可以批量选择删除对话
- [ ] 7. 渲染由两个`$$`包裹的公式块应该独占一行，由两个`$`包裹的公式块应该嵌入在文本中


## 1月26日：
- [x] 1. 排查解决React模式聊天异常结束没有发送结束节点的问题。
- [x] 2. 配置文件从本地`config.yaml`转为`Nacos`云端统一配置
- [x] 3. 能否将React默认提示词提供出来，可以由下游组装提示词或者上游直接组装
...
## 1月28日：
- [x] 1. 优化海大ReAct模式的提示词（流程部分）
- [x] 2. 根据 `Gemini Cli` 以及代码评审修改代码
- [x] 3. 完善健康检查接口和会话总结接口的返回值
- [x] 4. 修复ReAct模式聊天异常结束没有发送结束节点的问题（解决任务取消下的生产者与消费者问题）

## 1月29日：
- [x] 1. 优化海大项目提示词检索逻辑部分以及检索的语言
- [ ] 2. 学习 `.pre-commit-config.yaml` 文件的编写和运行逻辑
- [x] 3. 将全部的本地配置转换为Nacos云端，例如模型最大循环次数等


## 1月30日：
- [x] 1. 需要根据近几年的数据进行总结和预测和趋势推理（需要有例如：增长百分点数据），需要基于现有数据进行总结提炼
- [x] 2. 检索词汇，例如小龙虾（龙虾，等品种），需要一个问题重写？比如AI必须知道例如（小龙虾的学名叫克氏原螯虾），需要有检索词的联想
- [x] 3. 提示词的建设，需要让大模型预测用户想要问什么样得到问题。
- [x] 4. 前期的时候，预设回复的答案，后期根据大部分的答案进行回答。（根据用户的反馈，确定关联词汇有哪些。）
