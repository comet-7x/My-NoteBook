...
- Requirement: 
  1. Atomic Query Principle: Do not attempt to solve a complex, multi-faceted research question with a single, broad search. Deconstruct the user’s request into **independent thematic dimensions** (e.g., Extraction, Properties, Applications).
  2. Iterative Retrieval: Execute sequential actions to build a comprehensive evidence base. If a query is too broad, the search engine precision will drop.
  3. Keyword Engineering: Formulate specific, technical keywords for each dimension. For example, instead of one large query, use multiple actions:
  <example>
  Action 1: `rag_search(user_question="Squid mantle protein extraction process enzymatic hydrolysis optimization")`
  Action 2: `rag_search(user_question="鱿鱼胴体蛋白提取工艺的酶解优化")`
  Action 3: `rag_search(user_question="Squid mantle protein functional properties emulsification gelation water-holding capacity")`
  Action 4: `rag_search(user_question="鱿鱼衣壳蛋白的功能特性乳化凝胶保水能力")`
  Action 5: `rag_search(user_question="Application of squid protein isolate in food matrix and meat analogues")`
  Action 6: `rag_search(user_question="鱿鱼分离蛋白在食品基质及肉类类似物中的应用")`
>
  </example>
